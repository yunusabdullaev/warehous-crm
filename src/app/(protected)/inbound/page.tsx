...
import type { InboundRecord, Product, Location, PaginatedResponse } from "@/lib/types";
...
export default function InboundPage() {
    ...
    const fetchAll = async () => {
        setLoading(true);
        try {
            const [inbRes, pRes, lRes] = await Promise.all([
                api.get<PaginatedResponse<InboundRecord>>("/inbound?limit=100"),
                api.get<PaginatedResponse<Product>>("/products?limit=100"),
                api.get<PaginatedResponse<Location>>("/locations?limit=100"),
            ]);
            setRecords(inbRes.data.data || []);
    ...
    const canReverse = (r: InboundRecord) => {
    ...
    const handleReverse = async (id: string) => {
        ...
        setReversingId(id);
        try {
            await api.post(`/inbound/${id}/reverse`, { reason });
            toast("success", "Inbound reversed");
            fetchAll();
        } catch (err: unknown) {
        ...
        <div>
            <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => { reset(); setShowForm(!showForm); }}
                    ...
                    {showForm ? "Cancel" : "+ New Inbound"}
                </button>
            </div>

            {showForm && (
                <div className="bg-white rounded-xl shadow-sm border p-5 mb-6">
                    <h2 className="text-lg font-semibold mb-4">Create Inbound</h2>
                    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-1">Reference / Supplier</label>
                            <input {...register("reference")} className="w-full px-3 py-2 border rounded-lg text-sm" placeholder="PO-2024-001" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Lot No *</label>
                            <input {...register("lot_no", { required: "Lot No is required" })} className="w-full px-3 py-2 border rounded-lg text-sm" placeholder="LOT-2024-001" />
                            {errors.lot_no && <p className="text-red-500 text-xs mt-1">{errors.lot_no.message}</p>}
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Exp Date</label>
                            <input {...register("exp_date")} type="date" className="w-full px-3 py-2 border rounded-lg text-sm" />
                        </div>
                        ...
                        <tbody>
                            {records.map((r) => (
                                <tr key={r.id} className={`border-t hover:bg-gray-50 ${r.status === "REVERSED" ? "opacity-60" : ""}`}>
                                    ...
                                    <td className="px-4 py-3 text-right font-semibold text-emerald-600">+{r.quantity}</td>
