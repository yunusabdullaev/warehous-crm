...
func (h *Handler) Create(c *fiber.Ctx) error {
	...
	inb, err := h.service.Create(c.Context(), warehouseID, &req, userID)
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": err.Error()})
	}

	return c.Status(fiber.StatusCreated).JSON(ToResponse(inb))
}
...
func (h *Handler) GetByID(c *fiber.Ctx) error {
	...
	inb, err := h.service.GetByID(c.Context(), id)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{"error": "inbound record not found"})
	}
	return c.JSON(ToResponse(inb))
}
...
func (h *Handler) Reverse(c *fiber.Ctx) error {
	...
	inb, err := h.service.Reverse(c.Context(), id, req.Reason, userID, userRole)
	if err != nil {
		switch err {
		case ErrAlreadyReversed:
			return c.Status(fiber.StatusConflict).JSON(fiber.Map{"error": "this inbound record has already been reversed"})
		case ErrForbidden:
